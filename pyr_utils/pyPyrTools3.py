...
import pyPyrUtils3 as pyPyrUtils
#import pyPyrCcode
import math
...
class SFpyr(Spyr):
    ...
    def __init__(self, *args):    # (image, height, order, twidth)
        ...
        ctr = np.int32(numpy.ceil((numpy.array(dims)+0.5)/2))
...
class SCFpyr(SFpyr):
    ...
    def __init__(self, *args):    # (image, height, order, twidth)
        ...
        dims = numpy.array(self.image.shape)
        ctr = numpy.int32(numpy.ceil((numpy.array(dims)+0.5)/2))
        ...
        for i in range(ht):
            ...
            dims = numpy.array(lodft.shape)
            ctr = numpy.int32(numpy.ceil((dims+0.5)/2))
            lodims = numpy.int32(numpy.ceil((dims-0.5)/2))
            loctr = numpy.ceil((lodims+0.5)/2)
            lostart = numpy.int32(ctr - loctr)
            loend = lostart + lodims
