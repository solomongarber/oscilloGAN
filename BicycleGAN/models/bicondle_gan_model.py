...
class BiCondleGANModel(BaseModel):
    def name(self):
        return 'BiCondleGANModel'
    ...
    def initialize(self, opt):
        ...
        self.visual_names = ['real_A_encoded', 'real_B_encoded','real_B_random', 'fake_B_random', 'fake_B_encoded']
        ...
        if use_E:
            self.model_names += ['E']
            self.netE = networks.define_E(opt.output_nc+opt.input_nc, opt.nz, opt.nef, which_model_netE=opt.which_model_netE, norm=opt.norm, nl=opt.nl,
    ...
    def test(self, z0=None, encode=False):
        with torch.no_grad():
            if encode:  # use encoded z
                z0, _ = self.netE(torch.cat([self.real_A,self.real_B],1))
            if z0 is None:
    ...
    def forward(self):
        ...
        # get encoded z
        self.conditional_input=torch.cat([self.real_A_encoded,self.real_B_encoded],1)
        self.z_encoded, self.mu, self.logvar = self.encode(self.conditional_input)
        # get random z
        ...
        if self.opt.lambda_z > 0.0:
            self.mu2, logvar2 = self.netE(torch.cat([self.real_A_encoded,self.fake_B_random],1))  # mu2 is a point estimate
