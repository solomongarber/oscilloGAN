...
def define_G2(input_nc, output_nc, nlayers, ngd, nl='relu',use_dropout=False, init_type='xavier',gpu_ids=[]):
    nl_layer=get_non_linearity(layer_type=nl)
    netG2 = G2net(input_nc,output_nc,nlayers,ngd,nl_layer=nl_layer,use_dropout=use_dropout,gpu_ids=gpu_ids)
...
def define_G(input_nc, output_nc, nz, ngf,
             ...
             use_dropout=False,split_outer=False, init_type='xavier', gpu_ids=[], where_add='input', upsample='bilinear'):
    netG = None
    ...
    if which_model_netG == 'unet_128' and where_add == 'input':
        ...
        netG = G_Unet_add_all(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                              use_dropout=use_dropout, gpu_ids=gpu_ids, upsample=upsample, split_outer=split_outer)
    else:
        raise NotImplementedError(
...
class G_Unet_add_all(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False, gpu_ids=[], upsample='basic', split_outer=False):
        super(G_Unet_add_all, self).__init__()
        self.gpu_ids = gpu_ids
        self.nz = nz
        self.split_outer=split_outer
        # construct unet structure
        ...
        unet_block = UnetBlock_with_z(input_nc, output_nc, ngf, nz, unet_block,
                                      outermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, split_outer=split_outer)
        self.model = unet_block
...
class UnetBlock_with_z(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc, nz=0,
                 submodule=None, outermost=False, innermost=False,
                 norm_layer=None, nl_layer=None, use_dropout=False, split_outer=False, upsample='basic', padding_type='zero'):
        ...
        if outermost:
            if split_outer:
                #upconv1 = upsampleLayer(inner_nc*2, 2, upsample=upsample, padding_type=padding_type)
                upconv = upsampleLayer(inner_nc*2,outer_nc,upsample=upsample,padding_type=padding_type)
                down = downconv
                #up1 = [uprelu] + upconv1 + [nn.Tanh()]
                up = [uprelu] + upconv
